 ...
 */
export const NIMBLE_CLIENT_SOURCE = 'vercel-ai-sdk';
...
export interface NimbleClientOptions {
  ...
  timeout?: number;
  /** SDK retry count for transient failures (SDK default applies when unset). */
  maxRetries?: number;
}

/**
 * Build the Nimble client this package uses: the caller's key plus this
 * package's `X-Client-Source` attribution. Every tool that constructs its own
 * client goes through here so the attribution cannot drift between tools.
 */
